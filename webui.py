...
if not os.path.exists(cmd_args.model_dir) or not os.path.exists(os.path.join(cmd_args.model_dir, "config.yaml")):
    if cmd_args.version == "2":
        ...
        print(f"Model directory {cmd_args.model_dir} is incomplete for v2.5. Please prepare checkpoints manually.")
        sys.exit(1)
...
if IS_V25:
    v25_config_path = os.path.join(current_dir, "configs", "config_v2_5.yaml")
    tts = IndexTTS2(model_dir=cmd_args.model_dir,
                    cfg_path=v25_config_path,
                    use_bf16=cmd_args.fp16,
...
page_title = "IndexTTS 2.5 Demo" if IS_V25 else "IndexTTS 2 Demo"
page_paper = "2601.03888" if IS_V25 else "2506.21619"
page_heading = (
    "IndexTTS 2.5: Multilingual and Expressive Zero-Shot Text-to-Speech"
    if IS_V25
    else "IndexTTS2: A Breakthrough in Emotionally Expressive and Duration-Controlled Auto-Regressive Zero-Shot Text-to-Speech"
)

with gr.Blocks(title=page_title) as demo:
    mutex = threading.Lock()
    gr.HTML(f'''
    <h2><center>{page_heading}</h2>
<p align="center">
<a href='https://arxiv.org/abs/{page_paper}'><img src='https://img.shields.io/badge/ArXiv-{page_paper}-red'></a>
</p>
    ''')
    ...
    with gr.Tab(i18n("音频生成")):
        with gr.Row():
            ...
            with gr.Column():
                input_text_single = gr.TextArea(label=i18n("文本"),key="input_text_single", placeholder=i18n("请输入目标文本"), info=f"{i18n('当前模型版本')}{cmd_args.version}")
                if IS_V25:
