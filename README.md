...
**0.22.3**。完整功能、开发和打包说明见 [Desktop README](desktop/README.md)。

> 0.22.3 修复 Gradio 流式音频组件导致页面停在“加载中”的问题，加入中文数字/年份归一化自检；
> 生成/停止操作固定在窗口底部，语音生成与多角色页面的高级区域默认折叠，启动器也允许迁移输出和用户数据目录。
...
完整便携包会包含这项 Python 依赖；GitHub 的小体积应用层增量更新不会替换 Python/CUDA 运行时，
旧整合包若状态显示不可用，应安装新的完整便携包。桌面源码环境可执行
