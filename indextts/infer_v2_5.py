...
def select_gpt_inference_dtype(
    use_bf16: bool, use_deepspeed: bool, device: str, use_fp16: bool = False
):
    """Use the Windows DeepSpeed kernel's supported FP16 path instead of BF16."""

    precision = resolve_gpt_precision(
        use_fp16=use_fp16, use_bf16=use_bf16, device=device
    )
    if use_deepspeed and str(device).startswith("cuda"):
        return torch.float16
    if precision == "fp16":
        return torch.float16
    return torch.bfloat16 if precision == "bf16" else None
...
class IndexTTS2:
    def __init__(
            ...
            use_fp16=False, reference_device=None, reuse_spk_cond_for_emo=False,
            reference_cache_dir=None, reference_cache_namespace=""
    ):
        """
        Args:
            ...
            use_fp16 (bool): use FP16 for UnifiedVoice/GPT; mutually exclusive with BF16.
            reference_device (str): device for Wav2Vec2-BERT and CAMPPlus; ``cpu`` saves VRAM.
            reuse_spk_cond_for_emo (bool): reuse speaker conditioning for implicit default emotion.
        ...
        precision = resolve_gpt_precision(
            use_fp16=use_fp16, use_bf16=use_bf16, device=self.device
        )
        ...
        if self.reference_device != str(self.device):
            print(f">> Reference encoders: {self.reference_device}; synthesis: {self.device}")
        if self.reuse_spk_cond_for_emo:
            print(">> Fast default emotion enabled")
        ...
        self.dtype = select_gpt_inference_dtype(
            self.use_bf16, use_deepspeed, self.device, self.use_fp16
        )
        if use_deepspeed and self.dtype == torch.float16:
            print(">> DeepSpeed GPT precision: FP16 (Windows inference kernel compatible)")
        ...
        self.gpt.post_init_gpt2_config(
            ...
            kv_cache=True,
            half=self.dtype in {torch.float16, torch.bfloat16},
    ...
    def _get_emo_cond_emb(self, emo_audio_prompt, spk_cond_emb, reuse_spk_cond, verbose):
        if reuse_spk_cond:
            return spk_cond_emb
        return self._prepare_emotion_reference(emo_audio_prompt, verbose)
    ...
    @staticmethod
    def _load_audio_with_pyav(audio_path, target_sr=None):
        """Decode compressed audio without relying on a system ffmpeg.exe."""
        try:
            import av
        except ImportError as exc:
            raise RuntimeError("缺少内置音频解码组件 PyAV。") from exc

        try:
            with av.open(str(audio_path)) as container:
                stream = next((item for item in container.streams if item.type == "audio"), None)
                if stream is None:
                    raise RuntimeError("文件中没有音频轨道")

                decoded_frames = container.decode(stream)
                first_frame = next(decoded_frames, None)
                if first_frame is None:
                    raise RuntimeError("音频轨道为空")

                output_sr = int(
                    target_sr
                    or first_frame.sample_rate
                    or stream.codec_context.sample_rate
                    or 16000
                )
                resampler = av.AudioResampler(format="fltp", layout="mono", rate=output_sr)
                chunks = []

                def append_frame(frame):
                    for converted in resampler.resample(frame):
                        chunk = torch.from_numpy(converted.to_ndarray()).to(torch.float32)
                        if chunk.ndim == 1:
                            chunk = chunk.unsqueeze(0)
                        chunks.append(chunk)

                append_frame(first_frame)
            ...
            if not chunks:
                raise RuntimeError("解码后没有有效音频采样")
            return torch.cat(chunks, dim=1), output_sr
        except Exception as exc:
            if isinstance(exc, RuntimeError) and str(exc).startswith("文件中"):
                detail = str(exc)
            elif isinstance(exc, RuntimeError) and str(exc).startswith("音频轨道"):
                detail = str(exc)
            else:
                detail = f"{type(exc).__name__}: {exc}"
            raise RuntimeError(
                f"无法解码音频“{os.path.basename(str(audio_path))}”（{detail}）"
            ) from exc

    def _load_and_cut_audio(self,audio_path,max_audio_length_seconds,verbose=False,sr=None):
        compressed_suffixes = {".aac", ".m4a", ".mp4", ".webm", ".wma", ".ac3"}
        suffix = os.path.splitext(str(audio_path))[1].lower()

        if suffix in compressed_suffixes:
            audio, loaded_sr = self._load_audio_with_pyav(audio_path, target_sr=sr)
        else:
            try:
                if not sr:
                    samples, loaded_sr = librosa.load(audio_path)
                else:
                    samples, loaded_sr = librosa.load(audio_path, sr=sr)
                audio = torch.tensor(samples).unsqueeze(0)
            except Exception as librosa_error:
                try:
                    audio, loaded_sr = self._load_audio_with_pyav(audio_path, target_sr=sr)
                except Exception as pyav_error:
                    raise RuntimeError(
                        f"无法读取音频“{os.path.basename(str(audio_path))}”。"
                        "请上传有效的 WAV、FLAC、MP3、AAC 或 M4A 文件。"
                    ) from pyav_error
    ...
    def _split_atomic_pieces(self, text):
        pieces, pos = [], 0
        for match in self.SPLIT_PROTECTED_PATTERN.finditer(text):
            if match.start() > pos:
    ...
    def split_text_by_tokens(self, text, max_tokens, lang_prefix=""):
        ...
        for piece, atomic in self._split_atomic_pieces(text):
            ...
            for part in re.split(r'(?<=[，。！？、；：,\.!\?;:\n])', piece):
                ...
                current = ""
                for char in part:
                    if current and self._token_len(current + char) > budget:
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path, lang,
        ...
        reuse_spk_cond = self._should_reuse_spk_cond_for_emo(
            emo_audio_prompt, emo_vector, use_emo_text
        )
        ...
        spk_cond_emb, style, prompt_condition, ref_mel = (
            self._prepare_speaker_reference(spk_audio_prompt, verbose)
        )
        ...
        emo_cond_emb = self._get_emo_cond_emb(
            emo_audio_prompt, spk_cond_emb, reuse_spk_cond, verbose
        )
        ...
        for seg_text in segments:
            segment_token_weights.append(
                len(self.tokenizer.encode(seg_text, allowed_special='all'))
            )
...
class QwenEmotion:
    ...
    def normalize_content(self, content):
        """Accept vector dictionaries and Qwen label-style emotion outputs."""
...
if __name__ == "__main__":
    ...
    tts.infer(spk_audio_prompt=args.prompt_wav, text=args.text, lang=args.lang, output_path=args.output,
              verbose=True, text_normalization=args.text_normalization)
