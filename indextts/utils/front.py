...
def _patch_kaldifst_windows_path_handling():
    """Pass Windows paths to kaldifst using the active ANSI code page.

    kaldifst 1.7.x opens rule files through a narrow C++ stream on Windows.
    pybind11 encodes Python ``str`` arguments as UTF-8, which makes an existing
    FST look missing whenever the application is installed below a non-ASCII
    directory.  A ``bytes`` argument avoids the extra UTF-8 conversion and lets
    the Windows runtime interpret the path with its native code page.
    """
    if os.name != "nt":
        return
    ...
    if getattr(kaldifst, "_indextts_windows_path_patch", False):
        return

    native_text_normalizer = kaldifst.TextNormalizer

    def text_normalizer_with_native_path(rule):
        if isinstance(rule, str):
            rule = rule.encode("mbcs")
        return native_text_normalizer(rule)

    kaldifst.TextNormalizer = text_normalizer_with_native_path
    kaldifst._indextts_windows_path_patch = True
...
class TextNormalizer:
    ...
    def load(self):
        ...
        if platform.system() != "Linux":  # Mac and Windows
            _patch_kaldifst_windows_path_handling()
    ...
    def _protect_pronunciation_annotations(self, text: str):
        """Protect ``<text|pronunciation>`` annotations during normalization."""
        placeholders = {}

        def _idx_to_alpha(index):
            value = ''
            while True:
    ...
    def normalize(self, text: str) -> str:
        if not self.zh_normalizer or not self.en_normalizer:
            ...
            return ""
        text, pronunciation_placeholders = self._protect_pronunciation_annotations(text)
        ...
        return self._restore_pronunciation_annotations(result, pronunciation_placeholders)
